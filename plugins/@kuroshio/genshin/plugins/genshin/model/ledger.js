...
class Ledger extends base {
  ...
  async get () {
    ...
    await this.saveLedger(this.e.uid)
  ...
  async saveLedger (uid, ck = '') {
    ...
    let dbData = await global.dbHelper.get('genshin_user_ledger', {
      uid: [uid]
    })

    if (dbData) {
      dbData.data = NoteData
      dbData.update_time = new Date()
      await global.dbHelper.upsert("genshin_user_ledger", dbData)
    }else {
      dbData = {
        uid: uid,
      ...
      await global.dbHelper.create("genshin_user_ledger", dbData)
    }
    logger.info(`[札记查询][自动保存] uid:${uid} 原石数据已保存`)
    return NoteData
  }
  ...
  async ledgerTask (manual) {
    ...
    logger.info(`札记ck:${uids.length}个，预计需要${this.countTime(uids.length)} ${finishTime} 完成`)
