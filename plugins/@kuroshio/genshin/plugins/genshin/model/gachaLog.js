...
class GachaLog extends base {
  ...
  async logApi (param) {
    ...
    let res = await fetch(logUrl + logParam).catch((err) => {
      this.logger.error(`[获取抽卡记录失败] ${err}`)
    })
    if (!res || !res.ok) {
      return { retcode: 400 }
    }
  ...
  async updateLog () {
    ...
    if (res.retcode !== 0 || !res?.data?.list || res.data.list.length <= 0) {
      this.logger.debug(`${this.e.logFnc} ${res.message || 'error'}`)
      return false
    }

    this.logger.info(`${this.e.logFnc}[uid:${this.uid}] 开始获取：${this.typeName}记录...`)
    let all = []

    let logJson = await this.readJson()
  ...
  async getAllLog (ids, authkey, page = 1, endId = 0) {
    ...
    if (!res?.data?.list || res.data.list.length <= 0) {
      this.logger.info(`${this.e.logFnc}[uid:${this.uid}] 获取${this.typeName}记录完成，共${Number(page) - 1}页`)
    ...
    for (let val of res.data.list) {
      if (ids.get(String(val.id))) {
        this.logger.info(`${this.e.logFnc}[uid:${this.uid}] 获取${this.typeName}记录完成，暂无新记录`)
        return { hasErr: false, list }
      } else {
        list.push(val)
        endId = val.id
  ...
  async readJson() {
    ...
    let logJson = userData? userData.data : [];
    let ids = new Map()
  ...
  async writeJson(data) {
    ...
    if (userData) {
      ...
      delete userData.id
      await global.dbHelper.update('genshin_user_gachalog', {id: id}, userData)
    } else {
      userData = {
        uid: this.uid,
        user_id: this.e.user_id,
      ...
      await global.dbHelper.create('genshin_user_gachalog', userData)
    }
  }
  ...
  async getUid () {
    ...
    userData.forEach(u => logs.push(u.uid))
    ...
    let uidArr = []
    for (let uid of logs) {
      let gachaData = await global.dbHelper.list('genshin_user_gachalog', {
        uid: uid,
        user_id: this.e.user_id,
        type: 301
      },['uid','update_time'])

      gachaData.forEach(gacha => uidArr.push({
        uid: gacha.uid,
        mtimeMs: gacha.update_time
      }))
    }
    if (uidArr.length <= 0) {
      return false
  ...
  async analyse() {
    if (lodash.isEmpty(this.all)) {
      this.all = (await this.readJson()).list
    }
    let fiveLog = []
    let fourLog = []
    let fiveNum = 0
