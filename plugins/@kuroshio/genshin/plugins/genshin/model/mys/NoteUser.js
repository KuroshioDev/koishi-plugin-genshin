...
class NoteUser extends BaseModel {
  ...
  static async create (qq, data = null) {
    ...
    let user = await new NoteUser().get(qq, data)
    // 检查绑定uid (regUid)
  ...
  async delCk (ltuid = '', needRefreshCache = true) {
    ...
    for (let uid in ckData) {
      ...
      if (ltuid && ck.ltuid * 1 === ltuid) {
        ret.push(uid)
        await global.dbHelper.remove('genshin_user_cookie', ckData[uid].id)
        delete ckData[uid]
      }
    }
    // 刷新主ck并保存ckData
  ...
  // 内部方法：读取CK数据
  async _getCkData () {
    this.ckData = await gsCfg.getBingCkSingle(this.qq)
    return this.ckData
  }

  // 内部方法：写入CK数据
  async _saveCkData () {
    await gsCfg.saveBingCk(this.qq, this.ckData)
  }
}

exports.NoteUser = NoteUser
