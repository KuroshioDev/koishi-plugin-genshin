...
class MysInfo {
  ...
  static async initUserCk() {
    ...
    logger.info(`加载用户UID：${userCount}个，加入查询池`)
  }
  ...
  async checkCode(res, type, isSr) {
    ...
    switch (res.retcode) {
      ...
      case 10103:
        if (/(登录|login)/i.test(res.message)) {
          if (this.ckInfo.uid) {
            logger.info(`[ck失效][uid:${this.uid}][qq:${this.userId}]`)
            this.e.reply(`UID:${this.ckInfo.uid}，米游社cookie已失效`)
          } else {
            logger.info(`[公共ck失效][ltuid:${this.ckInfo.ltuid}]`)
    ...
    if (res.retcode !== 0) {
      logger.info(`[mys接口报错]${JSON.stringify(res)}，uid：${this.uid}`)
    }
    // 添加请求记录
