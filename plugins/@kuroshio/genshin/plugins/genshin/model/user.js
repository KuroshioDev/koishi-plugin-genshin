...
const {MysUser} = require( './mys/MysUser.js')
const MysInfo = require( './mys/mysInfo.js')

const logger = new Logger("genshin-model-user")
...
class User extends base {
  ...
  async bing () {
    ...
    if (!await this.checkCk(param)) {
      logger.info(`绑定cookie错误：${this.checkMsg || 'cookie错误'}`)
    ...
    if (flagV2) {
      ...
      if (userFullInfo?.data?.user_info) {
        ...
        logger.info(`绑定cookie错误：${userFullInfo.message || 'cookie错误'}`)
        await this.e.reply(`绑定cookie失败：${userFullInfo.message || 'cookie错误'}`)
        return
      }
    }

    logger.info(`${this.e.logFnc} 检查cookie正常 [uid:${this.uid}]`)

    await user.addCk(await this.getCk())

    logger.info(`${this.e.logFnc} 保存cookie成功 [uid:${this.uid}] [ltuid:${this.ltuid}]`)
  ...
  async loadOldData () {
    ...
    logger.info(logger.green('加载用户ck...'))
    ...
    logger.info(logger.green(`加载用户ck完成：${lodash.size(arr)}个`))
