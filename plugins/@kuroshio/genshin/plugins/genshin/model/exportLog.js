...
const logger = new Logger('ExportLog')
...
class ExportLog extends base {
  ...
  async exportJson () {
    ...
    logger.info(`${this.e.logFnc} 导出成功${this.uid}.json`)
  ...
  async exportXlsx () {
    ...
    logger.info(`${this.e.logFnc} 开始导出${this.uid}.xlsx`)
    ...
    logger.info(`${this.e.logFnc} 导出成功${this.uid}.xlsx`)
  ...
  async logXlsx () {
    ...
    for (let type in data) {
      ...
      gachLog.uid = uid
      gachLog.type = type
      await gachLog.writeJson(data[type])
