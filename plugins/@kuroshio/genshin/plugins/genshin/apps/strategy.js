...
const logger = new Logger("genshin-app-strategy")
gsCfg.cpCfg('mys', 'set')
...
class strategy extends plugin {
  constructor (ctx, session) {
    ...
    this.path = `${common.getDataPath()}/strategy`
  ...
  async getImg (name, group) {
    ...
    logger.info(`${this.e.logFnc} 下载${name}攻略图`)

    if (!await common.downFile(url + this.oss, this.sfPath)) {
      return false
    }

    logger.info(`${this.e.logFnc} 下载${name}攻略成功`)
