...
class Cfg {
  ...
  /** 初始化配置 */
  initCfg() {
    let path = `${common.getConfigPath()}/config/`
    let pathDef = `${common.getConfigPath()}/default_config/`
  ...
  /** 频道机器人appID */
  get qqBot() {
    return this.getConfig('qq').bot
  }

  /** 七牛配置*/
  get qiniu() {
    return this.getConfig('other').qiniu
  }
   ...
   */
  getYaml(type, name) {
    let file = `${common.getConfigPath()}/${type}/${name}.yaml`
    let key = `${type}.${name}`
